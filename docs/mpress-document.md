---
title: M-Press Flavoured Markdown specification
description: The strict Markdown profile used by M-Press, including metadata, components, diagnostics, and canonical formatting.
order: 13
---

M-Press Flavoured Markdown, or MPF, is the strict Markdown profile used by
M-Press. Source files keep the familiar `.md` extension. This document specifies
schema 1.

MPF is Markdown with a closed set of conventions for metadata, components, and
deterministic processing. It is not a competing document format and it does not
permit MDX or executable source.
...
MPF remains ordinary Markdown. Its stricter conventions give M-Press a
deterministic document model for builds, transformations, translations, and
editing.

## Markdown compatibility contract

Markdown interoperability is part of the format, not a separate convenience
tool. A conforming M-Press implementation must provide all of these guarantees:
...
M-Press Flavoured Markdown. A conforming Markdown adapter must implement the interchange
...
An M-Press Flavoured Markdown document must use UTF-8. A byte order mark is permitted only at
...
M-Press Flavoured Markdown has no required signature or preamble. This is a complete
schema 1 document:
...
A document without metadata uses schema 1. The `.md` extension identifies the
source as Markdown. The metadata `schema` field selects a different M-Press
schema when one is required.

## See the source and its rendering

The examples below are compiled by M-Press on this page. Select a tab to compare
the rendered result with the exact Markdown that produced it.
...
[Guidance]
@note{type=tip title="No framework required"}
The component is responsive, accessible, and themed by default.
@end
...
Keep the source readable in any text editor.

### Build the site

Generate complete static HTML with one command.
@end

[Terminal]
@terminal{title="Build the documentation" language=sh frame=macos}
$ mpress build --strict
Built 61 pages and 85 files in 79 ms
@end

[Table]
@table{header search filter sort paginate column-separators page-size=3}
| Component | Interactive | Included |
| Terminal | Yes | Yes |
| Steps | No | Yes |
| Rich table | Yes | Yes |
| File tree | No | Yes |
...
[Image]
@image{src="/images/mpress-hello-output.png" alt="A generated M-Press documentation site" expand}
...
The metadata body uses M-Press metadata syntax, not YAML. Each field occupies
...
reference syntax such as `[identifier]` is not part of M-Press Flavoured Markdown.
...
Canonical M-Press Flavoured Markdown writes the canonical shortcode name. A Markdown
...
M-Press Flavoured Markdown does not interpret HTML entities. Write the UTF-8 character itself.
...
Its opening line begins with `@`. A container body uses ordinary M-Press Flavoured Markdown,
and `@end` closes it.
...
not part of M-Press Flavoured Markdown. An unknown directive produces an `mpd-directive`
...
separate adjacent attributes. Braces are not part of M-Press Flavoured Markdown component
syntax.
...
Video is a leaf component backed by the browser's native video element. The
common case requires only a source filename:

```text
@video src="setup.mp4"
```

MPress adds native controls, inline playback, metadata preloading, a responsive
layout, a 1280 by 720 intrinsic size, the inferred MIME type, and a fallback
link. Authors do not repeat those defaults.

The same `src` attribute accepts an array when a video has multiple encodings.
The browser tries them in source order. `base` prefixes every relative media
path, including sources, the poster, captions, and transcript:
...
The optional attributes are deliberately limited:

- `base` removes a repeated directory from relative media paths.
- `poster` sets the image shown before playback.
- `captions` accepts one WebVTT filename or an array. The first track is the
  default track.
- `lang` supplies the caption language when it cannot be inferred. It defaults
  to `en`.
- `title` supplies the accessible name and visible caption.
- `transcript` adds a transcript link.
- `width` and `height` replace the intrinsic 1280 by 720 dimensions. CSS keeps
  the player responsive.

Caption filenames can include a BCP 47 language tag immediately before the
extension, such as `captions.en.vtt` or `captions.pt-BR.vtt`. MPress infers that
tag before it uses `lang`. Video MIME types are inferred from `.mp4`, `.m4v`,
`.webm`, `.ogv`, `.ogg`, and `.mov` filenames. Relative paths and HTTP or HTTPS
URLs are accepted. Unsafe URL schemes are rejected.

MPress does not add autoplay, custom playback controls, or a JavaScript player
API. A site that needs an unusual player policy can use `@rawHTML`; those cases
do not enlarge the portable video component.
...
An include reads another M-Press Flavoured Markdown file or fragment. An import invokes a
format adapter.
...
If a construct is valid Markdown but has no native M-Press Flavoured Markdown equivalent, the
...
| Markdown construct | M-Press Flavoured Markdown representation |
| --- | --- |
| Paragraph | Paragraph |
...
| YAML frontmatter | Delimited M-Press metadata with values converted without loss |
| Unsupported extension | Exact `opaque-markdown` node |

### Persisted provenance

Source files remain Markdown, so provenance refers directly to the `.md` file.
The parser records the original bytes, their digest, the selected Markdown
profile, and node ranges. Canonical formatting does not require a second source
format or a sidecar file.

An exporter must verify the source digest before applying preserved ranges. If
the digest does not match, it must use canonical mode or stop with an error. It
...
Canonical M-Press Flavoured Markdown:
...
- M-Press Flavoured Markdown export to Markdown followed by semantic re-import;
...
M-Press must not describe a schema as stable until its grammar, Markdown
adapter, and conformance suite have shipped together.
